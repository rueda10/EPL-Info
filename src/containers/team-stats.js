...
class TeamStats extends Component {
        ...
        if (this.props.leagueData) {
            ...
            const home = (
                <Item key={`${team.name}_home_form`}>
                    ...
                    <Item.Description>Losses <span className="team-stat-value">{team.home.losses}</span></Item.Description>
                    <Item.Description>Draws <span className="team-stat-value">{team.home.draws}</span></Item.Description>
                    <Item.Description>Goals For <span className="team-stat-value">{team.home.goals}</span></Item.Description>
                    <Item.Description>Goals Against <span className="team-stat-value">{team.home.goalsAgainst}</span></Item.Description>
                </Item>
            );
            ...
            const away = (
                <Item key={`${team.name}_away_form`}>
                    ...
                    <Item.Description>Losses <span className="team-stat-value">{team.away.losses}</span></Item.Description>
                    <Item.Description>Draws <span className="team-stat-value">{team.away.draws}</span></Item.Description>
                    <Item.Description>Goals For <span className="team-stat-value">{team.away.goals}</span></Item.Description>
                    <Item.Description>Goals Against <span className="team-stat-value">{team.away.goalsAgainst}</span></Item.Description>
                </Item>
            );
    ...
    renderTopPointsPerGame(teamId, players) {
        const currentTeamPlayers = players.filter((player) => {
            ...
            if (a.points_per_game < b.points_per_game) { return 1; }
            else if (a.points_per_game === b.points_per_game) { return 0; }
            else { return -1; }
        });

        const topPointsPerGame = currentTeamPlayers.slice(0, 5).map((player) => {
            if (player.points_per_game > 0) {
                return <Item.Description key={`${player.id} points per game`}>{player.first_name} {player.second_name} <span className="team-stat-value">{player.points_per_game}</span></Item.Description>
            }
        });
        ...
        return (
            <Item>
                ...
                { topPointsPerGame.every((element) =>  { return element === undefined}) ? 'No players with points' : topPointsPerGame }
            </Item>
        )
    }
    ...
    renderLeastGoalsConceded(teamId, players) {
        const currentTeamPlayers = players.filter((player) => {
            ...
            if (a.goals_conceded < b.goals_conceded) { return 1; }
            else if (a.goals_conceded === b.goals_conceded) { return 0; }
            else { return -1; }
        }).reverse();
        ...
        const leastGoalsConceded = currentTeamPlayers.filter((player) => {
            ...
            return <Item.Description key={`${player.id} goals conceded`}>{player.first_name} {player.second_name} <span className="team-stat-value">{player.goals_conceded}</span></Item.Description>
        });

        return (
            <Item>
                <Item.Header>Least Goals Conceded</Item.Header>
                { leastGoalsConceded.every((element) =>  { return element === undefined}) ? 'No players' : leastGoalsConceded }
            </Item>
        )
    }
